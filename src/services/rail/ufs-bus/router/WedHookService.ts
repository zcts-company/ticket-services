...
import { UFSOrder } from "../types/UFSOrder.js";
...
webHookService.post('/web-hook-load',asyncHandler( 
    async(req:any, res:Response) => {
        logger.info(`[UFS BUS] Resived post request for webhook`);
...
async function createFile(reservationData: UFSOrder, key:string, updated:Date) {
    ...
    const fileName = nameOfFile(key, updated, config.checkUpdates) + "_bus_hook.xml";
    const path = `${currentDirectory}${fileName}`
    fileService.writeFile(path,res).then(() => {
        
        logger.info(`[UFS BUS WEB HOOK] File with name ${fileName}.xml created by hand in directory: ${currentDirectory}`);
