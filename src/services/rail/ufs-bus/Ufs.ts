...
import { fileService} from "../../../instances/services.js";
import { logger } from "../../../common/logging/Logger.js";
import config from "../../../config/rail/ufs_bus.json" with {type: 'json'}
import mainConf from "../../../config/main-config.json" with {type: 'json'}
import { RailServiceServer } from "../interfaces/RailServiceServer.js";
import { UFSTransport } from "./transport/UFSTransport.js";

export class UfsBus implements RailServiceServer {
    ...
    async startServer(): Promise<void> {
        ...
        if(!directoryArhiveExist){
            ...
            logger.info(`[UFS_BUS] Directory created: ${this.archiveDirectory}`);
        }

        if(!directoryCurrentExist){
            await this.fileService.createDirectory(this.currentDirectory)
            logger.info(`[UFS_BUS] Directory created: ${this.currentDirectory}`);
        }

        if(!directory1CExist){
            await this.fileService.createDirectory(this.directory1C)
            logger.info(`[UFS_BUS] Directory created: ${this.directory1C}`);
        }
          
        setInterval(() => {
            logger.trace(`[UFS_BUS] Step transport service of ufs service`);
     ...
     public async setCurrentArchiveDirectory(date:Date):Promise<void>{      
        ...
        logger.info(`[UFS_BUS] Server recived date from archive path: ${date.toLocaleDateString()}`);
        this.currentArchiveDirectory = `${config.fileArhive.mainPath}${date.toLocaleDateString().replace(new RegExp('[./]', 'g'),"-")}/`;
        logger.info(`[UFS_BUS] Cerrent archive path setted: ${this.currentArchiveDirectory}`);
        ...
        if(!directoryArhiveExist){
            ...
            logger.info(`[UFS_BUS] Directory created by server methods: ${this.currentArchiveDirectory}`);
        }
