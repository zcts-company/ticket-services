...
import { PdfPageSplitService } from "./PdfPageSplitService.js";
...
export class BusPdfProcessingService {
    ...
    constructor(
        ...
        private readonly pageSplitService: PdfPageSplitService = new PdfPageSplitService()
    ) {
    }

    async processMail(mail: IncomingMail): Promise<ProcessedBusTicket[]> {
        const attachments = await this.attachmentService.extractPdfAttachments(mail.source);

        if (attachments.length === 0) {
            logger.info(`Email with UID ${mail.uid} ` + `does not contain PDF attachments`)
            return [];
        }

        const result: ProcessedBusTicket[] = [];
        ...
        for (const attachment of attachments) {
             ...
             */
            const pageNumbers = parsedDocuments.map((document) => this.getDocumentPageNumber(document.source.pageNumber, analysis.pageCount, attachment.filename));
            /*
             * Загружаем исходный PDF один раз и создаём
             * отдельные одностраничные PDF.
             */
            const separatedPages = analysis.pageCount === 1
                ? new Map<number, Uint8Array>([[1, attachment.content]])
                : await this.pageSplitService.extractPages(attachment.content, pageNumbers);

            for (let index = 0; index < parsedDocuments.length; index++) {
                const document = parsedDocuments[index];
                const pageNumber = pageNumbers[index];
                const pdfContent = separatedPages.get(pageNumber);

                if (!pdfContent) {
                    throw new Error(`Separated PDF page ` + `${pageNumber} was not created for ` + `"${attachment.filename}"`);
                }

                result.push({ document, pdfContent });
            }
        }
    ...
    private getDocumentPageNumber(pageNumber: number | undefined, pageCount: number, filename: string): number {
        ...
        throw new Error(`Parser did not provide pageNumber for ` + `multi-page PDF "${filename}"`);
    }
}