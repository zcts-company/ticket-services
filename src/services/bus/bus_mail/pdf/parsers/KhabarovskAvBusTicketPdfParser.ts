...
export class KhabarovskAvBusTicketPdfParser extends BaseRegexBusTicketPdfParser implements BusTicketPdfParser {
    ...
    detect(analysis: PdfAnalysisResult): PdfParserDetection {
        ...
        add(
            /Рейс\s+Номер\s+Платформа\s+Место\s+Перевозчик/i,
            "ETRAFFIC_ROUTE_TABLE",
            15
        );

        add(
            /сайт\s+e-traffic\.ru|www\.e-traffic\.ru/i,
            "E_TRAFFIC",
            10
        );
    ...
    parse(
        ...
        const explicitTotal = this.toMoney(
            this.extractFirstGroup(
                text,
        ...
        this.validateCriticalFields({
            parserName: "Khabarovsk AV E-traffic",
            receiptId,
            ticketNumber,
            routeName: route.routeName
        });
    ...
    private extractRouteData(
        ...
        const tripMatch = table.match(
            /(\d{1,6}[A-Za-zА-Яа-яЁё]?)\s+((?:Перрон|Платформа)\s+\d+|\d+)\s+Место\s+(\d+)/iu
        );
        ...
        return {
            ...
            platform: this.clean(tripMatch[2]),
            seat: tripMatch[3],
            carrierName: carrierMatch
                ? this.clean(carrierMatch[1])
                : undefined,
            carrierId
        };
    }
    ...
    private extractTripPoints(
        ...
        const beforeDeparture = this.clean(
            block.slice(0, departureIndex)
        );

        const betweenDates = this.clean(
            block.slice(
                departureIndex + departure[0].length,
                arrivalIndex
            )
        );
        ...
        const afterArrival = this.clean(
            block.slice(
                arrivalIndex + arrival[0].length
            )
        );

        let departureRaw = beforeDeparture;
        let arrivalRaw = this.clean(
            [betweenDates, afterArrival]
                .filter(Boolean)
                .join(" ")
        );
        ...
        if (
            ...
            departureRaw = this.clean(
                [beforeDeparture, afterArrival]
                    .filter(Boolean)
                    .join(" ")
            );

            arrivalRaw = betweenDates;
        }
