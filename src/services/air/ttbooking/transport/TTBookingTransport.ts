...
import config from "../../../../config/air/ttbooking.json" with {type: 'json'}

export class TTBookingTransport {
            ...
            if(files.length > 0){
                files.forEach(async (fileName) => {
                    try{
                            ...
                            if(exists){
                                logger.info(`[TTBOOKING TRANSPORT] File ${fileName} sended to directory: ${this.directory1C}`);
                                await this.sendToArchive(currentArchive,fileName)
                            }
                    }catch {
                        logger.error(`[TTBOOKING TRANSPORT] Directory ${this.directory1C} not exists or not available`)                        
                    }
            })
           }
                 ...
                 files.forEach(async (fileName) => {
                     try{
                        ...
                        const exists:boolean = await this.sambaClient.fileExists(config.samba.directory + fileName)
                             ...
                             if(exists){
                                 logger.info(`[TTBOOKING TRANSPORT] File ${this.currentDirectory + fileName} sended to directory (SAMBA SERVER:${config.samba.server}): ${config.samba.directory}`);
                                 await this.sendToArchive(currentArchive,fileName)
                             }
                     }catch (error:any) {
                         logger.error(`[TTBOOKING TRANSPORT] Directory (SAMBA SERVER:${config.samba.server}) ${config.samba.directory} not exists or not available`)
                         logger.error(`[TTBOOKING TRANSPORT] ERROR: ${error.message}`)
                     }
             })
            }
    ...
    private async sendToArchive(currentArchive:string,fileName:string){
        ...
        if(exists){
            logger.info(`[TTBOOKING TRANSPORT] File ${fileName} sended to archive directory: ${currentArchive}`);
    ...
    private async removeFileFromCurrent(fileName:string){
       ...
       const exist:boolean = await fs.pathExists(`${this.currentDirectory}${fileName}`)
            ...
            if(!exist){
                logger.info(`[TTBOOKING TRANSPORT] File ${fileName} removed from current directory: ${this.currentDirectory}`);
            }
    }
