...
import { HotelService } from "../interfaces/HotelService.js";
import { replaceSymbols } from "../../../util/stringFunction.js";
...
import { ListResponse, OrderEntry } from "./types/response_list/ListResponse.js";
...
export class Traveltech implements HotelService {
    ...
    private createFile(reservationData: OrderEntry | undefined, key: string, updated: Date) {
        ...
        if (reservationData) {
            ...
            // подготовка полей к сериализации в xml (guests и documents)
            // при сериализации поля типа array сериализуются как набор полей с одинаковыми ключами
            // например guests:[{},{},{}] при преобразовании в xml получим 3 поля с ключем guests
            // что бы получить guests и вложенные guest оборачиваем полученный массив в поле guest
            if (Array.isArray(reservationData.order.guests)) {
                reservationData.order.guests = { guest: reservationData.order.guests.map(g => g) };
            }

            // анаогично с полем documents
            if (Array.isArray(reservationData.order.documents)) {
                reservationData.order.documents = { document: reservationData.order.documents.map(d => d) };
            }
