    ...
    async sendTo1CSamba(currentArchive:string|undefined){
             ...
             if(files.length > 0){
                 files.forEach(async (fileName) => {
                     try{
                             ...
                             if(exists){
                                 logger.info(`[TRAVELTECH TRANSPORT] File ${this.currentDirectory + fileName} sended to directory (SAMBA SERVER:${config[this.profile].samba.server}): ${config[this.profile].samba.directory}`);
                                 await this.sendToArchive(currentArchive,fileName)
                             }
                     }catch (error:any) {
                         logger.error(`[TRAVELTECH TRANSPORT] Directory (SAMBA SERVER:${config[this.profile].samba.server}) ${config[this.profile].samba.directory} not exists or not available`)
                         logger.error(`[TRAVELTECH TRANSPORT] ERROR: ${error.message}`)
                     }
             })
            }
     ...
     async forceSendTo1CSamba(fileName:string, path:string){
        try{
            ...
            if(exists){
               logger.info(`[TRAVELTECH TRANSPORT] File ${path + fileName} sended to directory (SAMBA SERVER:${config[this.profile].samba.server}): ${config[this.profile].samba.directory}`);
