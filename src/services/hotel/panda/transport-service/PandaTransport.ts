...
import config from "../../../../config/hotel/panda.json" with {type: 'json'}
import { ProfileType } from "../../../../common/types/ProfileType.js"

export class PandaTransport {
            ...
            if(files.length > 0){
                files.forEach(async (fileName) => {
                    try{
                            ...
                            if(exists){
                                logger.info(`[PANDA TRANSPORT] File ${fileName} sended to directory: ${this.directory1C}`);
                                await this.sendToArchive(currentArchive,fileName)
                            }
                    }catch (error:any) {
                        logger.error(`[PANDA TRANSPORT] Directory ${this.directory1C} not exists or not available`)
                        logger.error(`[PANDA TRANSPORT] ERROR: ${error.message}`)
                    }
            })
           }
    ...
    async sendTo1CSamba(currentArchive:string|undefined){
             ...
             if(files.length > 0){
                 files.forEach(async (fileName) => {
                     try{
                             ...
                             if(exists){
                                 logger.info(`[PANDA TRANSPORT] File ${this.currentDirectory + fileName} sended to directory (SAMBA SERVER:${config[this.profile].samba.server}): ${config[this.profile].samba.directory}`);
                                 await this.sendToArchive(currentArchive,fileName)
                             }
                     }catch (error:any) {
                         logger.error(`[PANDA TRANSPORT] Directory (SAMBA SERVER:${config[this.profile].samba.server}) ${config[this.profile].samba.directory} not exists or not available`)
                         logger.error(`[PANDA TRANSPORT] ERROR: ${error.message}`)
                     }
             })
            }
     ...
     async forceSendTo1CSamba(fileName:string, path:string){
        try{
            ...
            if(exists){
               logger.info(`[PANDA TRANSPORT] File ${path + fileName} sended to directory (SAMBA SERVER:${config[this.profile].samba.server}): ${config[this.profile].samba.directory}`);
    ...
    private async sendToArchive(currentArchive:string,fileName:string){
        ...
        if(exists){
            logger.info(`[PANDA TRANSPORT] File ${fileName} sended to archive directory: ${currentArchive}`);
    ...
    private async removeFileFromCurrent(fileName:string){
       ...
       const exist:boolean = await fs.pathExists(`${this.currentDirectory}${fileName}`)
            if(!exist){
                logger.info(`[PANDA TRANSPORT] File ${fileName} removed from current directory: ${this.currentDirectory}`);
            }
    }
